...
const createCliente = async (req, res) => {
    try {
        ...
        const rutaCliente = await ruta_1.default.getRutaById(req.body.id_ruta);
        if (!rutaCliente) {
            return res.status(400).send({ error: 'La ruta especificada no existe' });
        }
        const nuevoCliente = await cliente_1.default.createCliente(req.body);
        if (!nuevoCliente) {
...
const getClientes = async (req, res) => {
    try {
        ...
        const clientes = await cliente_1.default.getClientes(sucursal_id);
...
const getClienteById = async (req, res) => {
    try {
        ...
        const clienteEncontrado = await cliente_1.default.getClienteById(id);
...
const getClientesBySucursal = async (req, res) => {
    try {
        const sucursal_id = parseInt(req.params.sucursal_id);
        const clientesEncontrado = await cliente_1.default.getClientesBySucursal(sucursal_id);
...
const getClientesByRuta = async (req, res) => {
    try {
        const existeRuta = await ruta_1.default.getRutaById(parseInt(req.params.id_ruta));
        if (!existeRuta) {
            return res.status(400).send({ error: 'La ruta especificada no existe' });
        }
        const id_ruta = parseInt(req.params.id_ruta);
        const clientesEncontrado = await cliente_1.default.getClientesByRuta(id_ruta);
        return clientesEncontrado.length === 0
...
const getClientesByRutaPrestamo = async (req, res) => {
    try {
        const id_usuario = parseInt(req.params.id_usuario);
        const existeUsuario = await usuario_1.default.getUsuarioById(id_usuario);
        if (!existeUsuario) {
            return res.status(400).send({ error: 'El cobrador especificado no existe' });
        }
        const rutaEncontrada = await AsignacionRuta_1.default.getRutaAsignadaUsuario(id_usuario);
        if (!rutaEncontrada) {
            return res.status(404).send({ message: 'El cobrador no tiene una ruta asignada' });
        }
        const clientesEncontrado = await cliente_1.default.getClientesByRutaPrestamo(rutaEncontrada.ruta_id);
        return clientesEncontrado.length === 0
            ? res.status(404).send({ message: 'Cliente no encontrado para la ruta' })
            : res.status(200).json(clientesEncontrado);
    }
...
const getClientesByUser = async (req, res) => {
    try {
        ...
        const clientesEncontrado = await cliente_1.default.getClientesByUser(id_usuario);
        return clientesEncontrado.length === 0
...
//getClientesRutaUser
const getClientesRutaUser = async (req, res) => {
    try {
        const id_usuario = parseInt(req.params.id_usuario);
        const existeUsuario = await usuario_1.default.getUsuarioById(id_usuario);
        if (!existeUsuario) {
        ...
        const clientesEncontrado = await cliente_1.default.getClientesRutaUser(id_usuario);
        return clientesEncontrado.length === 0
            ? res.status(404).send({ message: 'Cobrador no tiene asignada ruta o la Ruta no tiene clientes' })
            : res.status(200).json(clientesEncontrado);
    }
...
const getClientesConPrestamosActivos = async (req, res) => {
    try {
        const sucursal_id = parseInt(req.params.sucursal_id);
        const clientesEncontrado = await cliente_1.default.getClientesConPrestamosActivos(sucursal_id);
        return clientesEncontrado.length === 0
...
const updateCliente = async (req, res) => {
    try {
        ...
        const clienteActualizado = await cliente_1.default.updateCliente(req.body);
...
const deleteCliente = async (req, res) => {
    try {
        const id = parseInt(req.params.id);
        const clienteEliminado = await cliente_1.default.deleteCliente(id);
...
const actualizarOrdenClientes = async (req, res) => {
    try {
        ...
        const id_ruta = parseInt(req.params.id_ruta);
        const resultado = await cliente_1.default.actualizarOrdenClientes(id_ruta, clientesOrdenados);
